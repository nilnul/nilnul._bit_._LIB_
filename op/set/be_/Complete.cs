...
namespace nilnul.bit.of_.vec.set.be_
{
	/// todo: document|article
	/// <summary>
	/// if any positive-ary function can be expressed.
	/// nullary func may not be expressed.
	/// If we want to express nullary function <see cref="bit.abgebra.be_.Complete"/>
	/// </summary>
...
this(
		...
		void initProjection()
		{
			...
			///		(x,y) => x.
			///	To apply this func to a subexpr, and that doesnot add value, as we allow that a subexpr (except the var itself) to return directly.
			///	
			///		
			///	conjecture:
